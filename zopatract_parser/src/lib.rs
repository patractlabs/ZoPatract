...
#[derive(Parser)]
#[grammar = "zopatract.pest"]
struct ZoPatractParser;

pub fn parse(input: &str) -> Result<Pairs<Rule>, Error<Rule>> {
    ZoPatractParser::parse(Rule::file, input)
}
...
mod tests {
    ...
    mod examples {
        ...
        fn examples_dir() {
            ...
            for entry in
                glob("../zopatract_cli/examples/**/*.zop").expect("Failed to read glob pattern")
            {
                match entry {
                    Ok(path) => {
                        ...
                        assert!(ZoPatractParser::parse(Rule::file, &data).is_ok());
                    }
                    Err(e) => panic!("{:?}", e),
                }
            }
    ...
    mod rules {
        ...
        fn parse_for_loop() {
            ...
            let parse = ZoPatractParser::parse(Rule::iteration_statement, input);
            assert!(parse.is_ok());
        }
    }
}
