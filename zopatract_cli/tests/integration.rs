...
mod integration {
    ...
    use std::path::Path;
    use tempdir::TempDir;
    use zopatract_abi::{parse_strict, Encode};
    ...
    fn test_compile_and_witness(
        ...
        let inputs_abi: zopatract_abi::Inputs<zopatract_field::Bn128Field> =
            parse_strict(&json_input_str, signature.inputs)
                .map(|parsed| zopatract_abi::Inputs::Abi(parsed))
                .map_err(|why| why.to_string())
                .unwrap();
