...
use zopatract_fs_resolver::FileSystemResolver;
#[cfg(feature = "libsnark")]
use {
    zopatract_core::proof_system::libsnark::Libsnark, zopatract_core::proof_system::pghr13::PGHR13,
};
        ...
        true => {
            ...
            match is_abi {
                true => match stdin.read_to_string(&mut input) {
                    Ok(_) => {
                        use zopatract_abi::parse_strict;
...
fn cli() -> Result<(), String> {
    ...
    let default_stdlib_path = dirs::home_dir()
        .map(|p| p.join(".zopatract/stdlib"))
        .unwrap();

    // cli specification using clap library
    let matches = App::new("ZoPatract")
    ...
    .subcommand(SubCommand::with_name("compute-witness")
        ...
        .arg(Arg::with_name("input")
            ...
            .long("abi")
            .help("Use ABI encoding. Arguments are expected as a JSON object as specified at zopatract.github.io/toolbox/abi.html#abi-input-format")
...
mod tests {
    ...
    fn compile_examples() {
        for p in glob("./examples/**/*").expect("Failed to read glob pattern") {
            ...
            let stdlib = std::fs::canonicalize("../zopatract_stdlib/stdlib").unwrap();
