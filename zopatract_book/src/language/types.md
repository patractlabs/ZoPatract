...
Booleans are available in ZoPatract. When a boolean is used as a parameter of the main function, the program is constrained to only accept `0` or `1` for that parameter. A boolean can be asserted to be true using an `assert(bool)` statement.
...
ZoPatract provides two complex types: arrays and structs.

### Arrays

ZoPatract supports static arrays, i.e., whose length needs to be known at compile time.
...
ZoPatract offers a special shorthand syntax to initialize an array with a constant value:
`[value; repetitions]`
...
#### Spreads and Slices
ZoPatract provides some syntactic sugar to retrieve subsets of arrays.
...
This operation is called slicing, and the following example shows how to slice in ZoPatract:
```
field[3] a = [1, 2, 3]
